...
const wranglerSignals: ReadonlyArray<NodeJS.Signals> = ["SIGINT", "SIGTERM"] as const

const baselineListeners = globalValue(
  "@effect/platform-cloudflare/wrangler/baselineListeners",
  () => {
...
const originalToWrapped = globalValue(
  "@effect/platform-cloudflare/wrangler/originalToWrapped",
  () => new WeakMap<SignalHandler, SignalHandler>()
)

const wrappedToOriginal = globalValue(
  "@effect/platform-cloudflare/wrangler/wrappedToOriginal",
  () => new WeakMap<SignalHandler, SignalHandler>()
)
...
> =>
  Effect.acquireRelease(
    Effect.gen(function*() {
      const { getPlatformProxy } = yield* Effect.promise(() => import("wrangler"))
